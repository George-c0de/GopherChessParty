...
func (uq *UserQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent_repository: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		Aggregate(ent_repository.Count()).
//		Scan(ctx, &v)
...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range uq.inters {
		if inter == nil {
			return fmt.Errorf("ent_repository: uninitialized interceptor (forgotten import ent_repository/runtime?)")
		}
	...
	for _, f := range uq.ctx.Fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent_repository: invalid field %q for query", f)}
		}
	}
	if uq.path != nil {
