...
func main() {
	...
	repository := connection.MustNewRepository(cfg.Database)

	// Создание репозиториев
	userRepo := connection.NewUserRepository(log, repository)
	gameRepo := connection.NewGameRepository(log, repository)
